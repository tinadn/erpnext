...
class TestStockReorderReportExecute(FrappeTestCase):
	...
	def test_execute_with_item_code_filter(self):
	...
	def test_execute_with_include_uom(self):
		...
		conversion_col_index = [col["fieldname"] for col in columns].index("stock_uom")  # or separate if extended
		self.assertTrue(any(row[conversion_col_index] for row in data))


	def test_execute_shortage_qty_calculation(self):
