...
class TestStockReservationEntry(FrappeTestCase):
	...
	def test_validate_mandatory_raises_exception_for_missing_fields_TC_SCK_372(self):
		...
		for field in mandatory_fields:
			...
			with self.assertRaises(frappe.ValidationError, msg=f"Missing field {field} should raise ValidationError"):
	...
	def test_validate_throws_for_disabled_batch_TC_SCK_373(self):
		...
		sre.sb_entries = [
			frappe._dict({
				"idx": 1,
				"batch_no": "BATCH-DISABLED",
				"qty": 2
			})
		]

		# Step 5: Call method and expect validation error
		with self.assertRaises(frappe.ValidationError, msg="Should not allow reservation against disabled batch"):
			sre.validate_reservation_based_on_serial_and_batch()
	...
	def test_validate_batch_reservation_fails_if_partial_not_allowed_TC_SCK_374(self):
		...
		sre.sb_entries = [
			frappe._dict({
				"idx": 1,
				"batch_no": "LIMITED-BATCH",
				"qty": 3
			})
		]

		with self.assertRaises(frappe.ValidationError, msg="Should raise when qty > available and partial not allowed"):
			sre.validate_reservation_based_on_serial_and_batch()
	...
	def test_can_be_updated_raises_for_delivered_status_TC_SCK_376(self):
		...
		with self.assertRaises(frappe.ValidationError, msg="Should not allow updates when status is Delivered"):
			sre.can_be_updated()
	...
	def test_can_be_updated_raises_for_pick_list_source_TC_SCK_377(self):
		...
		with self.assertRaises(frappe.ValidationError, msg="Should not allow updates when created from Pick List"):
			sre.can_be_updated()
	...
	def test_validate_with_allowed_qty_throws_when_exceeds_allowed_TC_SCK_379(self):
		...
		with self.assertRaises(frappe.ValidationError, msg="Should throw if qty already reserved or delivered"):
	...
	def test_validate_with_allowed_qty_throws_if_qty_exceeds_allowed_TC_SCK_380(self):
		...
		with self.assertRaises(frappe.ValidationError, msg="Should raise if trying to reserve more than allowed qty"):
	...
	def test_create_sre_skips_group_warehouse_TC_SCK_383(self):
		...
		sre = frappe.db.exists("Stock Reservation Entry", {
			"voucher_type": "Sales Order",
			"voucher_no": so.name
		})
