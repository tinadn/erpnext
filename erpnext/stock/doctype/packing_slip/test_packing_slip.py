...
class TestPackingSlip(FrappeTestCase):
	...
	def test_validate_delivery_note_raises_on_non_draft_dn_TC_SCK_418(self):
		...
		parent_expenses = frappe.db.get_value("Account", {
		...
		cost_center=create_cost_center(cost_center_name="_Test Cost Center", parent_cost_center=parent_cost_center,company="_Test Company")
		warehouse = create_warehouse("_Test Warehouse", company="_Test Company")

		# Step 1: Create and submit a Delivery Note (so docstatus = 1)
		dn = create_delivery_note(qty=1,warehouse=warehouse,cost_center=cost_center,expense_account=expense_account)
		dn.submit()
	...
	def test_validate_case_nos_from_case_no_zero_TC_SCK_419(self):
		...
		# Step 1: Create and submit a Delivery Note (so docstatus = 1)
		self.dn = create_delivery_note(qty=1,warehouse=warehouse,cost_center=cost_center,expense_account=expense_account)
		ps = frappe.new_doc("Packing Slip")
	...
	def test_item_details_returns_expected_items_TC_SCK_421(self):
		from erpnext.stock.doctype.packing_slip.packing_slip import item_details
		from erpnext.stock.doctype.warehouse.test_warehouse import create_warehouse
		from erpnext.accounts.doctype.account.test_account import create_account
		from erpnext.stock.doctype.item.test_item import make_item
		...
		cost_center=create_cost_center(cost_center_name="_Test Cost Center", parent_cost_center=parent_cost_center,company="_Test Company")
		warehouse = create_warehouse("_Test Warehouse", company="_Test Company")
		# Step 1: Create item and Delivery Note
		item_code = make_item("_Test Item")
		dn = create_delivery_note(item=item_code,qty=1,warehouse=warehouse,cost_center=cost_center,expense_account=expense_account)
	...
	def test_calculate_net_total_pkg_raises_for_mismatched_uom_TC_SCK_422(self):
		...
		ps.append("items", {
			...
			"qty": 1,
			"net_weight": 3.0,
			"weight_uom": "g"  # <-- different UOM to trigger the error
		})
	...
	def test_set_missing_values_sets_weight_fields_TC_SCK_423(self):
		...
		item_code = make_item("_Test Item WGT", {
			"weight_per_unit": 2.0,
			"weight_uom": "Kg",
			"is_stock_item": 1
		})
		...
		ps.append("items", {
			"item_code": item_code,
			"qty": 1,
			"net_weight": 0,      # should get updated
			"weight_uom": None    # should get updated
		})
	...
	def test_validate_items_throws_for_invalid_qty_and_missing_references_TC_SCK_424(self):
		...
		ps.items[0].qty = 1
		with self.assertRaises(frappe.ValidationError, msg="Should raise error for missing dn_detail/pi_detail"):
			ps.validate_items()
