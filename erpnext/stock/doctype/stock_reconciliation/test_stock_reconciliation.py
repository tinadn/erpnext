...
class TestStockReconciliation(FrappeTestCase, StockTestMixin):
	...
	def test_has_change_in_serial_batch_detects_difference(self):
		...
		if not frappe.db.exists("Item", "_Test Item Serial"):
			...
			frappe.db.set_value("Item", "_Test Item Serial", {
				"has_serial_no": 1,
				"serial_no_series": "SRL-.#####",
				"maintain_stock": 1
			})		
		...
		row = frappe._dict({
			"serial_and_batch_bundle": bundle_1.name,
			"current_serial_and_batch_bundle": bundle_2.name
		})		
	...
	def test_update_valuation_rate_for_serial_no_applies_to_all_serials(self):
		...
		serial_nos = frappe.get_all("Serial No", filters={"item_code": "_Test Item UVRSN"}, pluck="name", limit=2)
		serial_no_str = "\n".join(serial_nos)
	...
	def test_update_valuation_rate_for_serial_nos_respects_docstatus_and_rate(self):
		...
		serial_no = frappe.get_all("Serial No", filters={"item_code": "_Test Item UVRSN2"}, pluck="name", limit=1)[0]

		# Prepare row with valuation_rate and current_valuation_rate
		row = frappe._dict({
			"valuation_rate": 750,
			"current_valuation_rate": 300
		})
	...
	def test_merge_similar_item_serial_nos_merges_correctly(self):
		...
		sl_entries = [
			...
			frappe._dict({  # This should not merge due to missing serial no
				"item_code": "_Test Item",
				"warehouse": "_Test WH - _TC",
				"serial_no": "",
				"actual_qty": 3,
			...
			frappe._dict({  # This should not merge due to negative quantity
				"item_code": "_Test Item",
				"warehouse": "_Test WH - _TC",
				"serial_no": "SRL-0003",
				"actual_qty": -1,
		...
		merged = [entry for entry in result if "\n" in (entry.serial_no or "")][0]
...
def create_batch_or_serial_no_items():
	...
	serial_item_doc = make_item("Stock-Reco-Serial-Item-1", {"is_stock_item":1})
	if not serial_item_doc.has_serial_no:
		serial_item_doc.has_serial_no = 1
		serial_item_doc.serial_no_series = "SRSI.####"
		serial_item_doc.save(ignore_permissions=True)

	serial_item_doc = make_item("Stock-Reco-Serial-Item-2",  {"is_stock_item":1})
	if not serial_item_doc.has_serial_no:
		serial_item_doc.has_serial_no = 1
		serial_item_doc.serial_no_series = "SRSII.####"
		serial_item_doc.save(ignore_permissions=True)

	batch_item_doc = make_item("Stock-Reco-batch-Item-1",  {"is_stock_item":1})
	if not batch_item_doc.has_batch_no:
		batch_item_doc.has_batch_no = 1
		batch_item_doc.create_new_batch = 1
