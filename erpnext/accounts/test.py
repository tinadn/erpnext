...
class QueryPaymentLedger:
	...
	def query_for_outstanding(self):
		...
		if self.limit and self.get_invoices:
			...
			# frappe.log_error("common_filter", common_filter_dict)
		...
		# execute SQL
		frappe.log_error("self.cte_query_voucher_amount_and_outstanding",self.cte_query_voucher_amount_and_outstanding)
		self.voucher_outstandings = self.cte_query_voucher_amount_and_outstanding.run(as_dict=True,debug=1)
